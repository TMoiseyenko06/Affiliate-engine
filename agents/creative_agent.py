...
AFFILIATE_SCENE_SYSTEM_PROMPT = (
    ...
    "its job in context.\n\n"
    "Critically, the image must NOT look like an advertisement or a product "
    "catalog photo. It should look like an authentic, candid photo a real "
    "person casually took in their own home while actually using the thing — "
    "natural, slightly imperfect framing (not perfectly centered/symmetrical), "
    "real ambient clutter and context around it, phone-camera-like realism "
    "rather than professional studio lighting or a glossy commercial finish. "
    "Before writing the prompt, think about the specific person who would stop "
    "scrolling for this: what relatable moment, feeling, or 'before/after' "
    "contrast would make them go 'wait, I need to see this'? Build the scene "
    "around that — e.g. mid-task realism, a satisfying transformation visible "
    "in frame, or a small relatable moment of chaos being solved — rather than "
    "a static showcase.\n\n"
    "Concrete examples of the target feel: labeled spice jars on a real spice "
    "rack among other labeled jars and food, a hand caught mid-motion "
    "sprinkling spice onto food while actually cooking; storage bins shown "
    "genuinely full of food inside an open, lived-in refrigerator (not "
    "staged-empty); a drawer organizer shown filled with utensils in an "
    "actually-open, slightly-in-use drawer. Respond with a single JSON object: "
    ...
    "negative space, minimalist compositions, and anything that reads as a "
    "staged commercial/advertisement. The scene should fill the entire frame "
    "with a real, lived-in, candidly-photographed environment."
)
...
def _static_fallback_prompt(content_type: str, copy: Dict[str, Any], subject: Dict[str, Any]) -> str:
    ...
    if content_type == "affiliate":
        ...
        return (
            f"Candid, authentic-looking photo showing '{title}' actively being used "
            f"in a real, lived-in {category} setting, in context with other relevant "
            f"real objects around it — like a real person's phone photo, NOT a "
            f"staged advertisement or product catalog shot, NOT a white or empty "
            f"background, NOT professional studio lighting. Full-bleed scene filling "
            f"the entire frame. Features to reflect: {features}. Themes: {keywords}. "
